...
Diese Buchstaben kann man in die Ergebnisabbildung plotten oder als Superskript in einer Ergebnistabelle der Mittelwerte. Die @fig-zincconcentration zeigt ein Beispiel. Hier unterscheiden sich nur *High* und *Low* signifikant voneinander, da dies das einzige Paar ist, das keine gemeinsamen Buchstaben hat.

![(aus Quinn & Keough 2002)](./images/media/image29.png){#fig-zincconcentration width=50% fig-align="center"}
...
Im Fall von *t*-Tests bzw. ANOVAs ist die einfachste Möglichkeit, nach Faktorlevels gruppierte Boxplots zu betrachten. Alternativ gingen auch Histogramme, allerdings sind diese nur bei grossen *n* aussagekräftig (@fig-voraussetzungen).

:::{#fig-voraussetzungen layout-row=1}
![](./images/media/image31.png){#fig-voraussetzungen-a}

![](./images/media/image32.png){#fig-voraussetzungen-b}

![](./images/media/image33.png){#fig-voraussetzungen-c}

Boxplots und Histogramme
:::

Für die **Beurteilung der Varianzhomogenität** betrachtet man am besten die Höhe der Boxen im Boxplot. Wenn sie ähnlich hoch sind, ist alles OK, wenn sie sehr stark abweichen, hat man evtl. ein Problem. Sehr stark meint aber, siehe @fig-voraussetzungen, wirklich sehr stark, d. h. wenn die Box in einer Kategorie mehr als 4-mal so hoch ist wie in einer anderen (bei gleichen/ähnlichen Replikatzahen), und ab mehr als doppelt so hoch bei erheblich verschiedenen Replikatzahlen. Im vorliegenden Fall ist die Varianz in Gruppe 1 etwa 2.5-mal so hoch wie in Gruppe 2, da die Zahl der Replikate aber identisch war, wäre das noch OK.
...
2. Wenn dagegen die **Varianzen sehr heterogen** sind, die **Residuen aber relativ normal/symmetrisch**, wie in @fig-varianz-heterogen, kann der **Welch-Test** eingesetzt werden:

![Boxplot](./images/media/image28.png){#fig-varianz-heterogen width=50% fig-align="center" fig-pos='htbp'}
...
@fig-tranformationen-1 bis @fig-tranformationen-3 visualisieren exemplarisch die Effekte unterschiedlicher Transformationen auf die Werteverteilung (ganz links sind jeweils die untransformierten Daten, die Transformation rechts hat jeweils eine deutlich bessere Annäherung an die Normalverteilung erzielt).

![(Quinn & Keough 2002)](./images/media/image34.png){#fig-tranformationen-1 width=80% fig-align="center"}

![(Quinn & Keough 2002)](./images/media/image35.png){#fig-tranformationen-2 width=80% fig-align="center"}

![(Quinn & Keough 2002)](./images/media/image36.png){#fig-tranformationen-3 width=80% fig-align="center"}
...
Bei der Pearson-Korrelation betrachtet man die beiden Parameter Kovarianz (reicht von −∞ bis +∞) und die Korrelation, welche die Covarianz auf den Bereich von --1 bis +1 standardisiert. Pearsons Korrelationskoeffizient r ist der Schätzer für die Korrelation basierend auf der Stichprobe (@fig-pearson-korr).

![(aus Quinn & Keough 2002)](./images/media/image42.png){#fig-pearson-korr width=70% fig-align="center"}
...
Es mag vielleicht zunächst überraschen, aber ähnlich wie beim Vergleich von Mittelwerten zwischen kategorischen Ausprägungen kategorischer Variablen, liegt auch der linearen Regression eine **Varianzanalyse** zugrunde (@fig-varianzanalyse).

:::{#fig-varianzanalyse layout-ncol="1"}

![](./images/media/image44.png){#fig-varianzanalyse-1 width=90% fig-align="center"}

![](./images/media/image45.png){#fig-varianzanalyse-2 width=50% fig-align="center"}
...
![](./images/media/lm-summary.png)


Wir benötigen:

1. **Name des Verfahrens (Methode)**: Einfache lineare Regression (mit der Methode derkleinsten Quadrate).
2. **<span custom-style="red">[Signifikanz (Verlässlichkeit des Ergebnisses)]{.textcolor arguments="red"}</span>**: p-Wert der Steigung, nicht der p-Wert des Achsenabschnittes (wird nach üblicher Konvention auf drei Nachkommastellen gerundet oder, wenn unter 0.001, dann als p < 0.001 angegeben).
3. **<span custom-style="blue">[Effektgrösse und -richtung (unser eigentliches Ergebnis!)]{.textcolor arguments="blue"}</span>**: Im Falle einer linearen Regression ist das die Funktionsgleichung, die sich aus den Schätzungen der Koeffizienten ergibt.
4. **<span custom-style="green">[Erklärte Varianz (Relevanz des Ergebnisses)]{.textcolor arguments="green"}</span>**: Wie viel der Gesamtvariabilität der Daten wird durch das Modell erklärt? Ob $R^2$ oder $R^2_{adj.}$ angegeben werden sollte, wird unterschiedlich gesehen, jedenfalls sollte man explizit sagen, was gemeint ist. R² ist übrigens der quadrierte Wert von Pearsons Korrelationskoeffizienten $r$.
5. **ggf. Wert der Teststatistik mit den Freiheitsgraden („Zwischenergebnisse“)**: $F_{1,8} = 11.34$.

Ein adäquater Ergebnistext könnte daher wie folgt lauten:

:::{.callout}
Die Variable *b* nahm hochsignifikant mit der Variablen *a* ab (Funktionsgleichung: $b = 25.60 +(-0.26323) * a$ , $F_{1,13} = 28.03$, $p = 0.0001453$, $R^2 = 0.6588$.
:::
